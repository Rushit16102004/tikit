document.addEventListener('DOMContentLoaded', () => {
  ...
  const displayCin = document.getElementById('displayCin');
  const displayGstin = document.getElementById('displayGstin');
  ...
  // Developer Input Controls
  const devMovie = document.getElementById('devMovie');
  const devCinema = document.getElementById('devCinema');
  const devCinemaAddress = document.getElementById('devCinemaAddress');
  const devCin = document.getElementById('devCin');
  const devGstin = document.getElementById('devGstin');
  ...
  const devUserId = document.getElementById('devUserId');
  const devWorkstation = document.getElementById('devWorkstation');
  ...
  function loadConfig() {
    ...
    if (params.has('cin')) cfg.cin = params.get('cin');
    if (params.has('gstin')) cfg.gstin = params.get('gstin');
  ...
  function renderTicket() {
    ...
    const cinVal = cfg.cin || 'U59130GJ2006PTC049677';
    const gstinVal = cfg.gstin || '24AADCR4726N1Z4 / SAC: 999615';
    ...
    // Seats auto-sync: If seats not specified, auto generate seats
    let seats = cfg.seats;
    if (!seats) {
    ...
    displayCin.textContent = cinVal.startsWith('CIN:') ? cinVal : `CIN: ${cinVal}`;
    displayGstin.textContent = gstinVal.startsWith('GSTIN:') ? gstinVal : `GSTIN: ${gstinVal}`;
    ...
    displayBookingId.textContent = bookingId.startsWith('Booking ID') ? bookingId : `Booking ID - ${bookingId}`;
  ...
  function openDevConfigModal() {
    ...
    devCinemaAddress.value = cfg.cinemaAddress || '';
    devCin.value = cfg.cin || '';
    devGstin.value = cfg.gstin || '';
    ...
    // Auto-update seat input if empty
    ...
    devUserId.value = cfg.userId || '';
    devWorkstation.value = cfg.workstation || '';
  ...
  function updateDevTotalPreview() {
    ...
    // Auto update seat list input when ticket count changes
  ...
  function saveDevConfig() {
    const updated = {
      ...
      cinemaAddress: devCinemaAddress.value.trim(),
      cin: devCin.value.trim(),
      gstin: devGstin.value.trim(),
      date: devDate.value,
      time: devTime.value,
      ...
      userId: devUserId.value.trim(),
      workstation: devWorkstation.value.trim(),
